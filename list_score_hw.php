...
<?php 
		...
		$std = "select * from tb_student59 where year_in = '".$_POST['level']."' order by  room asc, ordinal asc ";
		echo $std;
...
?>
		...
		<td>
			<a onclick="open_pop('<?php echo $_POST['id_hw']; ?>','<?php echo $rs_std['student_id']; ?>')" class="w3-btn w3-indigo" >
		ตรวจการบ้าน</a> 
		...
		<?php //ส่งการบ้าน 
				...
				<?php }else{  if($rs_radio['total']>0){ ?>  
				<?php }else{ ?>
				 <?php }  }  echo $rs_radio['sum(quality_score)']; ?>
		
		</td>
		<td class="w3-centered w3-text-green ">
		<?php
